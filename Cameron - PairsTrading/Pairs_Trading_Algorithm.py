...
class DancingYellowGreenZebra(QCAlgorithm):
    ...
    def Initialize(self):
        ...
        #             'BFR', 'CM', 'COF', 'C', 'VLY', 'WFC', 'WAL', 'WBK','RBS', 'SHG', 'STT', 'STL', 'SCNB']
        ...
        # tickers = ["AAPL","AAL","AMD","AMZN","BA","BABA","BAC","C","CAT","COST","CSCO","CVX","DIS","DLR",
        #             "EQIX","F","GM","GOOGL","GS","IBM","INTC","JNJ","JPM","KO","LMT","LULU", "MCO", "META",
        #             "MPC", "MRK","MRNA","MSFT","MU","NFLX","NKE","NVDA","PFE","TSLA","VZ","WFC","WM","XOM",
        #             "SCHW", "RIOT", "XOM"]

        tickers = ['XOM', 'CVX', "EQIX", "MCO", "NEM", "KGC"]
        ...
        self.pairs = {}
        self.formation_period = 252 # Number of trading days in a year
    ...
    def OnData(self, data):
        ...
        for i in self.sorted_pairs:
            ...
            # the position is closed when prices revert back or stop loss is hit
            elif (self.Portfolio[i[0]].Invested and self.Portfolio[i[1]].Invested) or abs(spread[-1]) > mean + self.stop_loss * std:
                    self.Liquidate(i[0]) 
                    self.Liquidate(i[1])

    def Rebalance(self):
        # schedule the event to fire to select pairs with the smallest historical distance
        # if self.count % 6 == 0: # every six months
        if self.count % 1 == 0: # every month
            distances = {}
